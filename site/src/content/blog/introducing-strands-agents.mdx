...
The next diagram shows an architecture where the agent and its tools are deployed behind an API in production. We have provided reference implementations on GitHub for how to deploy agents built with the Strands Agents SDK behind an API on AWS, using [AWS Lambda](https://strandsagents.com/latest/user-guide/sdk/deploy/deploy_to_aws_lambda/), [AWS Fargate](https://strandsagents.com/latest/user-guide/sdk/deploy/deploy_to_aws_fargate/), or [Amazon Elastic Compute Cloud (Amazon EC2)](https://strandsagents.com/latest/user-guide/sdk/deploy/deploy_to_amazon_ec2/).
