...
The [agents-as-tools](/docs/user-guide/sdk/multi-agent/agents-as-tools/) pattern transforms specialized agents into intelligent tools that other agents can call, enabling hierarchical delegation where agents acting as the orchestrator dynamically consult domain experts without giving up control of the request. This mirrors how human teams work—a project manager doesn't need to know everything, they just need to know which specialist to consult for each task.
...
SYSTEM_PROMPT = """
Answer the user's support query. Use the stop tool after providing the final answer.
"""
...
Learn more about [Graphs](/docs/user-guide/sdk/multi-agent/graph/) in the Strands documentation.
...
data_analyst = Agent(name="analyst", tools=[notebook, shell])

@tool
def data_analyst_tool(query: str) -> str:
    return str(data_analyst(query))

analyst_orchestrator = Agent(tools=[notebook, data_analyst_tool]) # Agents-as-tools

# Compose patterns together - a graph that uses a swarm
researcher = Agent(name="researcher", tools=[notebook])
writer = Agent(name="writer", tools=[file_editor])
...
async def chat_endpoint(message: str):
    async def stream_response():
        agent = Agent(tools=[web_search, notebook])
...
Learn more about [Native Async Support](/docs/user-guide/sdk/streaming/async-iterators/) in the documentation.
...
The Strands community has been a critical voice in shaping all of these updates, through usage, feedback and direct code contributions. Of the over 150 PRs merged into Strands between 0.1.0 and 1.0, 22% were contributed by community members who fixed bugs, added model providers, wrote docs, added features, and refactored classes to improve performance. We're deeply grateful to [each of you](https://github.com/strands-agents/harness-sdk/graphs/contributors) for helping make Strands the simplest way to take an agent from prototype to production.
