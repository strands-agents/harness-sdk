...
- **`DakeraMemoryStore`** — a [`MemoryStore`](../../user-guide/sdk/memory/overview.md) that plugs into the agent loop via a `MemoryManager` (Strands ≥ 1.45), with automatic memory injection and extraction.
