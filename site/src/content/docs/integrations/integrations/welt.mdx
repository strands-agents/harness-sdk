...
Both Strands Harness SDKs have an adapter: [`welt-io-strands`](https://github.com/iwamot/welt-io-strands) for Python, [`@welt-io/strands`](https://github.com/iwamot/welt-io-strands-ts) for TypeScript. The four functions are the same on both sides, in each language's naming.
...
Strands' ready-made [`HumanInTheLoop`](../../user-guide/sdk/agents/interventions/human-in-the-loop.md) intervention also works over Welt as-is.
...
Adapter and front end ship as a pair: while both are 0.x, an adapter 0.Y release supports Welt v0.Y, and from 1.0 on any release sharing a major version. Support is best effort, and other combinations come with no guarantee. The Strands Harness SDK versions each adapter is built against are stated in its README.
