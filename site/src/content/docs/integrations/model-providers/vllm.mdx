...
```bash
pip install strands-vllm strands-agents
```

For retokenization drift demos (requires HuggingFace tokenizer):

```bash
pip install "strands-vllm[drift]" strands-agents
```
...
Strands Harness SDK already handles unknown tools and malformed JSON gracefully. `VLLMToolValidationHooks` adds RL-friendly enhancements:
...
result = agent("Pause for one second using the sleep tool.")
print(result)
```
...
async def main():
    ...
    await agent.invoke_async(
        'Create a notebook named "training" with three RL experiment ideas.'
    )
