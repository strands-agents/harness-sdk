...
Wire diagnosis into the `Experiment` class to diagnose cases automatically during evaluation. Pass a `DiagnosisConfig` to the experiment.
...
def notes_agent():
    return Agent(
        tools=[notebook],
        system_prompt="Use the notebook tool to organize the user's notes.",
        callback_handler=None,
    )
...
cases = [
    Case(
        name="basic-notes",
        input='Create a notebook named "ideas" and add three project ideas.',
        expected_output="Three ideas were added to the ideas notebook.",
        metadata={"task_description": "Notebook updated successfully"},
    ),
]
...
async def main():
    report = await experiment.run_evaluations_async(notes_agent, max_workers=1)

    # The experiment returns one combined report across all evaluators;
