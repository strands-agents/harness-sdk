...
All evaluators inherit from the base `Evaluator` class, which provides the structure for evaluation. Implement `evaluate`; the base class supplies a default `evaluate_async` that runs `evaluate` in a thread via `asyncio.to_thread`, so override `evaluate_async` only when you have a native async path (for example, an async LLM call):
...
The only method a subclass must implement. The base `evaluate` raises `NotImplementedError`.
...
Optional. The base class provides a default that delegates to `evaluate` through `asyncio.to_thread`, so an evaluator with only a synchronous `evaluate` works in the async path unchanged. Override it to run genuinely asynchronous work, such as an async LLM call.
...
2. **Implement `evaluate`**: Override `evaluate_async` only when you have a native async path; otherwise the inherited default covers it
