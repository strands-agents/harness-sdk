...
Chaos testing evaluates agent resilience by injecting controlled failures into tool execution. With `ChaosPlugin`, `ChaosCase`, and `ChaosExperiment`, you test how agents handle tool timeouts, network errors, and corrupted responses without changing agent code. See the [complete example](https://github.com/strands-agents/harness-sdk/blob/main/site/docs/examples/evals-sdk/chaos_tool_testing.py).
...
> **Note:** Each tool can only have **one effect** per `ChaosCase`. Passing multiple effects for the same tool (e.g., `"my_tool": [Timeout(), NetworkError()]`) raises a `ValueError`. To test multiple failure modes for a single tool, create separate `ChaosCase` instances, one per effect.
...
Setting `include_no_effect_baseline=True` adds an extra variant of each base case with no effects applied. This gives you a clean comparison point: you see how the agent scores under normal conditions versus under each failure scenario, so you can measure the delta that chaos introduces.
