...
flowchart LR
    ...
    B --> C[BidiResponseInterruptEvent]
...
The interruption flow: Model's VAD detects user speech → `BidiResponseInterruptEvent` sent → Audio buffer cleared → Response terminated → User's speech transcribed → Model ready for new input.
...
**BidiResponseInterruptEvent** - Emitted when interruption detected:
- `reason`: `"user_speech"` (most common) or `"error"`

**BidiResponseStopEvent** - Includes interruption status:
- `stop_reason`: `"end_turn"`, `"interrupt"`, `"error"`, or `"tool_use"`
...
class InterruptionTracker:
    ...
    async def on_interruption(self, event: BidiResponseInterruptHookEvent):
