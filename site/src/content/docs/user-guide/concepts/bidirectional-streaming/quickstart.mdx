...
`ConsoleIO`, and audio processing. Local microphone and speaker access also requires
...
implement custom I/O handlers using the `InputStream` and `OutputStream` protocols. See
[I/O Streams](io.md) for details.
:::
...
AudioIO uses PyAudio, which does not support echo cancellation. A headset is required
to prevent audio feedback loops.
