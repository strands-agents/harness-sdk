...
Send text, audio, or images with `agent.send()`. It accepts a string, a `TextBlock`,
`AudioBlock`, or `ImageBlock`, or a dictionary containing exactly one `text`,
`audio`, or `image` key.
...
Send audio bytes using an audio content block. The model configuration determines
the sample rate and channel count for real-time PCM audio.
