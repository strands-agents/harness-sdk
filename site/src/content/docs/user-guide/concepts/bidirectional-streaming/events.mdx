...
title: Events
description: >-
  Send and receive bidirectional streaming events for audio, text, tool activity,
  interruptions, usage, and persistent connection lifecycle.
experimental: true
tags: [bidi-streaming]
sourceLinks:
...
Tool events separate visibility from execution authorization. A model provider streams
each tool call first, then emits one event that closes the provider-defined group.

#### ToolUseStreamEvent

Use `ToolUseStreamEvent` to display or log a tool call as it arrives. This event does
not authorize execution. The agent loop waits for `BidiToolUsesCompleteEvent`.
...
Use `BidiToolUsesCompleteEvent` to identify the complete group that the agent loop
executes. The event contains one assistant message with every tool call in provider
order.

```python
{
    "type": "bidi_tool_uses_complete",
    "message": {
        "role": "assistant",
        "content": [
            {
...
The provider defines the group boundary. Gemini Live closes the group after its
tool-call list. OpenAI Realtime closes it from the completed `response.done` output.
Nova Sonic closes one call at the matching TOOL `contentEnd` event with
`stopReason=TOOL_USE`; Nova groups are not response-wide.

After the boundary, `BidiAgent` runs the group through its configured tool executor,
records one assistant tool-use message and one user tool-result message, then submits
the complete result group to the model provider.

#### ToolResultEvent and ToolResultMessageEvent

`ToolResultEvent` reports an individual result as each tool finishes.
`ToolResultMessageEvent` reports the ordered user message that contains the complete
result group. Concurrent execution can finish out of order, but the grouped message
retains the provider's original tool-call order.
    ...
    async with agent as agent:
        ...
        async for event in agent.receive():
            ...
            if event_type == "bidi_transcript_complete":
                ...
                print(f"Tool visible: {tool_use['name']}")
                print(f"Input: {tool_use['input']}")
            
            elif event_type == "bidi_tool_uses_complete":
                tool_count = len(event["message"]["content"])
                print(f"Executing tool group with {tool_count} call(s)")
