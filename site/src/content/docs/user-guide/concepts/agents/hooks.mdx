...
- [`BeforeInvocationEvent`](@api/python/strands.hooks.events#BeforeInvocationEvent)
    - `messages` - Mutable. Inspect, modify, replace, or remove the normalized input
      ...
      [Modify invocation messages](#modify-invocation-messages).
...
```typescript
--8<-- "user-guide/concepts/agents/hooks_imports.ts:modify_invocation_messages_imports"

--8<-- "user-guide/concepts/agents/hooks.ts:modify_invocation_messages"
```
