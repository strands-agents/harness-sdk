...
/> and skip manual configuration.
See [Context Management](../context-management). When
<Syntax
  py="context_manager"
  ts="contextManager"
/> is set, the ContextManager owns all context reduction
and any co-provided
<Syntax
...
### Context window limit, token estimation, and utilization

Proactive compression relies on the model's context window
limit, token counting, and utilization estimation. These
are shared across both conversation managers and the
ContextManager. See
[Context Estimation](../context-management/context-estimation)
for details on how the SDK resolves the context window
limit, estimates token counts, and computes utilization.
