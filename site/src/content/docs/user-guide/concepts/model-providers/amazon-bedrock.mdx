...
The Python SDK applies a default `read_timeout` of 120 seconds when constructing the Bedrock Runtime client, since botocore defaults to 60 seconds, which large or non-streaming requests can exceed. The default also applies when you pass your own `boto_client_config` without a `read_timeout`; set one explicitly to override it:
...
Passing `read_timeout=None` explicitly disables the timeout entirely.
