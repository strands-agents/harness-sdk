...
| [Handoff to User](#handoff-to-user) | Pause the agent loop and surface a message to the user | Python |
...
The loop halts with `stop_reason == "interrupt"` and the message is available
...
`HumanInTheLoop(allowed_tools=["handoff_to_user"])` so only the handoff itself
prompts.
:::
...
`handoff_to_user` is not yet supported in the TypeScript SDK.
