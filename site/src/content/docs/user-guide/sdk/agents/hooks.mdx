...
Hooks are composable and type-safe, and every event type accepts multiple subscribers. A *hook event* marks a point in the agent lifecycle; a *hook callback* is the function you register to run when that event fires.

Hooks cover tasks such as:
...
You register a hook callback against an event type, and it receives a strongly-typed event object each time that event fires. Each event carries the data for its stage of the lifecycle: `BeforeInvocationEvent` includes the agent and request details, and `BeforeToolCallEvent` provides the tool and its parameters.
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:individual_callback"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:orchestrator_callback"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/plugins/index.ts:plugin_for_hooks"
```
</Tab>
</Tabs>
...
### Modify invocation messages

To sanitize input before it enters conversation history, modify the `messages`
property on `BeforeInvocationEvent`. The SDK normalizes the current invocation input
before the event runs. Changes made by the hook become the input appended to history
...
def redact_input(event: BeforeInvocationEvent) -> None:
    if not event.messages:
        return

    event.messages[-1]["content"] = [{"text": "[REDACTED]"}]

agent.add_hook(redact_input, BeforeInvocationEvent)
agent("Contact me at user@example.com")
```
</Tab>
<Tab label="TypeScript">

```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:modify_invocation_messages_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:modify_invocation_messages"
```

The live event exposes `messages`, but `toJSON()` omits them.
</Tab>
</Tabs>

Assign a new collection to `event.messages` to replace the input. Assign an empty
collection to remove the input from the invocation.

## Hook events

Hook events fire at defined points across the agent loop and, for orchestrators, across node execution. For the lifecycle diagrams, the full catalogue of event types, and the properties you can modify on each, see the [Hook events](hooks-events.md) reference.
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:hook_ordering_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:hook_ordering"
```
...
Invocation state carries configuration and context data through the agent or orchestrator invocation. Reach for it when a hook needs to:

1. **Access custom objects**: database client objects, connection pools, or other Python objects
2. **Read request context**: session IDs, user information, settings, or request-specific data
3. **Share state across a multi-agent run**: access state shared across all agents, see [Shared State Across Multi-Agent Patterns](../multi-agent/multi-agent-patterns.md#shared-state-across-multi-agent-patterns)
4. **Pass custom parameters**: any additional data the hook needs
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:invocation_state_in_hooks"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:tool_interception_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:tool_interception"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:result_modification_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:result_modification"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:conditional_node_execution"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:composability"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:logging_modifications_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:logging_modifications"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:orchestrator_agnostic_design"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:layered_hooks"
```
</Tab>
</Tabs>

Layering the two gives you observability and control at both levels: individual agent execution and orchestrator coordination.

## Cookbook

Practical hook implementations for common tasks.
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:fixed_tool_arguments_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:fixed_tool_arguments_usage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:limit_tool_counts_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:limit_tool_counts_usage"
```
</Tab>
</Tabs>
...
Useful for implementing custom retry logic for model invocations. The `AfterModelCallEvent.retry` field allows hooks to request retries based on any criteria: exceptions, response validation, content quality checks, or custom logic. This example retries on exceptions with exponential backoff:
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:model_call_retry_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:model_call_retry_usage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:tool_call_retry_class"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks.ts:tool_call_retry_usage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:summarize_after_tools_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:summarize_after_tools"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:iterative_refinement_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:iterative_refinement"
```
</Tab>
</Tabs>
...
# so the caller receives the final result directly
...
```typescript
--8<-- "user-guide/sdk/agents/hooks_imports.ts:auto_approve_interrupts_imports"

--8<-- "user-guide/sdk/agents/hooks.ts:auto_approve_interrupts"
```
</Tab>
</Tabs>
