...
Context accumulates as the loop runs. Each iteration adds to the conversation history, so the model sees not just the original request but every tool it has called and every result it has received. That growing context is what lets the model reason across many steps.
...
See [Production lifecycle controls](./lifecycle-controls.md) for the stop reasons, invocation limits, and cancellation below collected in one place, with a worked example.
...
- **End turn** (<Syntax py="end_turn" ts="endTurn" />): The model has finished its
  response and has no further actions to take. This is the normal successful termination.
  The loop exits and returns the model's final message.
- **Tool use** (<Syntax py="tool_use" ts="toolUse" />): The model wants to execute one or
  more tools before continuing. The loop executes the requested tools, appends the results
  to the conversation history, and invokes the model again.
- **Cancelled** (`cancelled`): The agent was stopped externally via `agent.cancel()`. See
  [Cancellation](#cancellation) below.
- **Limit turns** (<Syntax py="limit_turns" ts="limitTurns" />): The per-invocation turn
  budget was exhausted. See [Invocation Limits](#invocation-limits).
- **Limit total tokens** (<Syntax py="limit_total_tokens" ts="limitTotalTokens" />): The
  cumulative token budget was exhausted. See [Invocation Limits](#invocation-limits).
- **Limit output tokens** (<Syntax py="limit_output_tokens" ts="limitOutputTokens" />):
  The output token budget was exhausted. See [Invocation Limits](#invocation-limits).
- **Max tokens** (<Syntax py="max_tokens" ts="maxTokens" />): The model's response was
  truncated because it hit the token limit. This is unrecoverable within the current loop.
  The model cannot continue from a partial response, and the loop terminates with an
  error.
- **Stop sequence** (<Syntax py="stop_sequence" ts="stopSequence" />): The model
  encountered a configured stop sequence. Like end turn, this terminates the loop
  normally.
- **Content filtered** (<Syntax py="content_filtered" ts="contentFiltered" />): The
  response was blocked by safety mechanisms.
- **Guardrail intervention**
  (<Syntax py="guardrail_intervened" ts="guardrailIntervened" />): A guardrail policy
  stopped generation.
...
<Tabs>
  ...
  ```typescript
  --8<-- "user-guide/sdk/agents/agent-loop.ts:cancel_timeout"
  ```
  ...
  ```typescript
  --8<-- "user-guide/sdk/agents/agent-loop.ts:cancel_external_signal"
  ```
  ...
  ```typescript
  --8<-- "user-guide/sdk/agents/agent-loop.ts:cancel_signal"
  ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  ```typescript
  --8<-- "user-guide/sdk/agents/agent-loop.ts:limits_basic"
  ```
...
<Tabs>
  ...
  To reject overlapping calls on shared conversation history, keep the default
  `concurrentInvocationMode: 'singleThreaded'`. A second call receives
  `ConcurrentInvocationError` while the agent is running.

  To deduplicate an in-flight retry, pass the same `idempotencyToken` in the options
  argument to `invoke()` or `stream()`. The retry waits for the original and receives
  the same result or error without rerunning the agent. A deduplicated `stream()` emits
  only the final `AgentResultEvent`, not the original's intermediate events.

  If the primary stream is abandoned before producing a result, waiting retries receive
  `IdempotencyAbortedError`. The token is forgotten when the invocation finishes, so it
  can be reused later. A different token, or a call without a token, still receives
  `ConcurrentInvocationError` while work is in flight.

  To allow overlapping calls, construct the agent with
  `concurrentInvocationMode: 'unsafeConcurrent'`. This mode ignores idempotency tokens
  and does not protect shared agent state from interleaved mutation. Use it only with
  your own synchronization; a separate agent per independent task is safer.
  </Tab>
</Tabs>
...
When the model consistently picks the wrong tool, the problem is usually ambiguous tool descriptions. Review the descriptions from the model's perspective. If two tools have overlapping descriptions, the model has no basis for choosing between them. See [Create custom tools](../tools/custom-tools.md#write-effective-tool-descriptions) for guidance on writing effective descriptions.
