...
The `"session"` preset captures `messages`, `state`, `conversation_manager_state`, `interrupt_state`, and `model_state`. It does not include `system_prompt` by default. See [Field Selection](#field-selection) to customize which fields are captured.
</Tab>
<Tab label="TypeScript">

```typescript
--8<-- "user-guide/sdk/agents/snapshots_imports.ts:imports"

--8<-- "user-guide/sdk/agents/snapshots.ts:take_snapshot"
```
...
--8<-- "user-guide/sdk/agents/snapshots.ts:load_snapshot"
```
</Tab>
</Tabs>

Restoring only touches fields present in the snapshot. If you left a field out when you took the snapshot, the agent keeps its current value for that field.

:::caution[Snapshots are restored verbatim]
A snapshot's `messages` are loaded into the agent as-is, including any tool-call
content they carry. Load snapshots only from a source you control. A snapshot
influenced by an untrusted party is untrusted input: its content reaches the
model, and in the Python SDK a trailing tool-call block runs a tool directly on
the next invocation. See
[Trusted Message History](../safety-security/trusted-message-history.md).
:::

## Field Selection

Select fields through presets, includes, and excludes. The resolution order is **preset → include → exclude**: start with the fields defined by the preset, add any fields listed in `include`, then remove any fields listed in `exclude`.
...
A `BidiAgent` supports `messages`, `state`, and `system_prompt`; its `"session"` preset
captures `messages` and `state`. Requesting another field raises `SnapshotException`, and
`load_snapshot` requires the agent to be stopped. For persistence, see
[bidirectional streaming sessions](../bidirectional-streaming/session-management.mdx).
...
--8<-- "user-guide/sdk/agents/snapshots.ts:field_selection"
```
</Tab>
</Tabs>
...
Snapshots carry an <Syntax py="app_data" ts="appData" /> field for storing application-owned data alongside the agent state. Strands passes this data through verbatim, without reading or modifying it.
...
--8<-- "user-guide/sdk/agents/snapshots.ts:app_data"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/agents/snapshots_imports.ts:imports_snapshot"

--8<-- "user-guide/sdk/agents/snapshots.ts:serialization"
```
</Tab>
</Tabs>
...
--8<-- "user-guide/sdk/agents/snapshots.ts:checkpointing"
```
</Tab>
</Tabs>
...
--8<-- "user-guide/sdk/agents/snapshots.ts:branching"
```
</Tab>
</Tabs>
...
[Session management](./session-management.mdx) handles persistence automatically: it saves and restores state at the right lifecycle points without you writing any persistence code. The snapshot-based session managers (`SnapshotSessionManager` in Python, `SessionManager` in TypeScript) persist these same snapshots to a [Storage](../storage.mdx) backend and support time-travel restore from immutable checkpoints. Snapshots give you manual control: you decide when to capture state, what to include, and where to store it. Use session management when you want hands-off persistence, and snapshots when you need precise control over save points.
...
- [Conversation Management](./conversation-management.mdx): managing conversation history and the context window
