...
AWS Lambda runs your code without provisioning or managing servers and bills only for the compute time you consume. That fits short-lived agent invocations: no hosts to manage, and nothing to pay while the agent is idle. This guide packages a Python agent handler and deploys it with the AWS CDK.
...
This Lambda deployment example does not implement response streaming as described in the [Async Iterators for Streaming](../streaming/async-iterators.md) documentation. If you need streaming capabilities, consider using the [AWS Fargate deployment](deploy_to_aws_fargate.md) approach which does implement streaming responses.
:::
...
The official Lambda layer bundles the base `strands-agents` package, so you can attach it to your function instead of building your own layer:
...
If you need packages beyond the base `strands-agents` SDK, create a custom layer
([`AgentLambdaStack.ts`][AgentLambdaStack]):
...
When using [Model Context Protocol (MCP)](../tools/mcp-tools.md) tools with Lambda, there are important considerations for connection lifecycle management.
