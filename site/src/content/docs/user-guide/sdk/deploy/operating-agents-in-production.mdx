...
Configure the agent explicitly rather than relying on defaults. The settings below cover the model, tools, and safety boundaries you want pinned down before an agent serves real traffic.

### Agent Initialization

Initialize agents with explicit configuration tailored to your production requirements rather than relying on defaults.

#### Model configuration

Pass a model with the specific configuration properties you want:
...
See:
 ...
 - [Bedrock Model Usage](../model-providers/amazon-bedrock.md/#basic-usage)
 - [Ollama Model Usage](../model-providers/ollama.md/#basic-usage)

### Tool Management

Control which tools the agent can reach in production:
...
See [Adding Tools to Agents](../tools/using-tools.md#adding-tools-to-agents) and [Auto reloading tools](../tools/using-tools.md#auto-loading-and-reloading-tools) for more information.
...
3. **Output Sanitization**: Sanitize outputs for sensitive information. Use [guardrails](../safety-security/guardrails.md) as an automated mechanism.
...
2. **Tool invocations**: Put quotas around tools that are costly, rate-limited,
   or have side effects. The [Limit Tool Counts hook](../agents/hooks.md#limit-tool-counts)
...
See [Invocation Limits](../agents/agent-loop.md#invocation-limits) for
turn and token budgets, [Cancellation](../agents/agent-loop.md#cancellation)
for timeout patterns, and [Stop Reasons](../agents/agent-loop.md#stop-reasons)
...
The [`SlidingWindowConversationManager`](../agents/conversation-management.md/#slidingwindowconversationmanager) helps prevent context window overflow exceptions by maintaining a reasonable conversation history size.

### Streaming for Responsiveness

Stream responses with `stream_async()` to deliver content to the caller as it arrives, which lowers perceived latency in production applications:
...
See [Async Iterators](../streaming/async-iterators.md) for more information.

### Error Handling

Handle errors explicitly in production: log the failure and fall back rather than letting an exception propagate to the caller.
...
* **Bedrock AgentCore** - A secure, serverless runtime for deploying and scaling dynamic AI agents and tools. [Learn more](deploy_to_bedrock_agentcore/index.md)
...
Monitor these signals for every production deployment:
...
See [Observability](../observability-evaluation/observability.md) for more information.

## Summary

Running an agent in production comes down to pinning its configuration, restricting its tools, bounding its execution, and watching it once it serves traffic. Pick the deployment pattern that fits your workload, wire up error handling and observability, and monitor how often each limit is reached so you can tune it.

## Related Topics

- [Conversation Management](../agents/conversation-management.md)
- [Streaming - Async Iterator](../streaming/async-iterators.md)
- [Tool Development](../tools/index.md)
- [Guardrails](../safety-security/guardrails.md)
- [Responsible AI](../safety-security/responsible-ai.md)
