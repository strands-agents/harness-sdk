...
Amazon Elastic Kubernetes Service (EKS) runs and scales your containerized applications on Kubernetes while AWS operates the Kubernetes control plane.

This guide uses [Amazon EKS Auto Mode](https://aws.amazon.com/eks/auto-mode), which extends that management to the compute, scaling, and networking infrastructure your workloads run on. That suits agents that run as containerized applications and need high availability and scale.

This guide discusses EKS integration at a high level. For a complete example project deploying to EKS, see the [`deploy_to_eks` sample project on GitHub][project_code].
...
Stream the response to send content to the client as the agent produces it, rather than waiting for the full answer. This matters most for long responses, where the wait to first token is otherwise noticeable.
...
The implementation above employs a [custom tool](../tools/custom-tools.md#creating-custom-tools) to mark the boundary between information gathering and summary generation phases. This approach ensures that only the final, user-facing content is streamed to the client, maintaining consistency with the non-streaming endpoint while providing the benefits of incremental response delivery.

## Containerization

To deploy your agent to EKS, you need to containerize it using Podman or Docker. The Dockerfile defines how your application is packaged and run. The example below installs the dependencies and application code and configures the FastAPI server to run via Uvicorn ([Dockerfile][dockerfile]):
