...
AWS Fargate is a serverless compute engine for containers that works with Amazon ECS and EKS, running your containers without servers or clusters to manage. It suits agents that need streaming responses, high concurrency, or high availability. This guide runs a containerized FastAPI agent on Fargate and deploys it with the AWS CDK.
...
To deploy your agent to Fargate, you need to containerize it using Podman or Docker. The Dockerfile defines how your application is packaged and run. The example below installs the dependencies and application code and configures the FastAPI server to run via Uvicorn ([Dockerfile][dockerfile]):
