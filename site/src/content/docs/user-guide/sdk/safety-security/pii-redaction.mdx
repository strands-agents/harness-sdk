...
Keep personal data out of your agent's telemetry by redacting it before traces and logs leave your system. Strands does not redact PII inside its own telemetry generation, so you add masking either through a third-party library or at the OpenTelemetry Collector. This page shows both.
...
Strands Harness SDK does not natively perform PII redaction within its core telemetry generation but recommends two effective ways to achieve PII masking:

### Option 1: Using Third-Party Specialized Libraries [Recommended]
Use a specialized external library such as Langfuse, LLM Guard, Presidio, or AWS Comprehend for high-quality PII detection and redaction:
