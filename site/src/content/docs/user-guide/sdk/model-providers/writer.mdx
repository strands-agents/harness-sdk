...
[Writer](https://writer.com/) is an enterprise generative AI platform offering specialized Palmyra models for finance, healthcare, creative, and general-purpose use cases. The models support tool calling, structured output, and domain-specific tasks, with Palmyra X5 offering a 1M token context window.
...
> responses to stdout as they're generated. When you call `agent(...)`, you'll see
> the response appear in real time. The `print(response)` call above also shows the
> final collected result. See [Callback Handlers](../streaming/callback-handlers.md)
> for more details.
...
> **Note**: The `web_search` and `email_sender` tools in this example are custom tools
> that you would need to define. See [Python Tools](../tools/custom-tools.md) for guidance
> on creating custom tools, or use the SDK's [vended tools](../tools/vended-tools.md).
