...
The [`OllamaModel`](@api/python/strands.models.ollama) class connects to Ollama's API and supports:
...
Ollama supports structured output for models that have tool calling capabilities. When you pass `structured_output_model` parameter to the [agent invocation](@api/python/strands.agent.agent#Agent.__call__), the Strands Harness SDK converts your Pydantic models to tool specifications that compatible Ollama models can understand.
