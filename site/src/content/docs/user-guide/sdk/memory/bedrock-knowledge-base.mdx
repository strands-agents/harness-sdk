...
`BedrockKnowledgeBaseStore` is a [`MemoryStore`](./managing-memory#stores) backed by [Amazon Bedrock Knowledge Bases](https://docs.aws.amazon.com/bedrock/latest/userguide/knowledge-base.html).
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:read_only_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:read_only"
```
</Tab>
</Tabs>

To enable writing memories through the [`add_memory` tool](./managing-memory#memory-tools) or [automatic extraction](./managing-memory#automatic-extraction), mark the store writable and point it at a data source that accepts ingestion:
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:writable_custom_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:writable_custom"
```
</Tab>
</Tabs>
...
The outer `BedrockKnowledgeBaseStoreConfig` carries the per-store identity and behavior, plus the [shared `MemoryStore` fields](./managing-memory#stores) (`name`, `description`, <Syntax py="max_search_results" ts="maxSearchResults" />, `writable`, `extraction`):
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:scoped_stores_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:scoped_stores"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:s3_store_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:s3_store"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:search_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:search"
```
</Tab>
</Tabs>
...
Enable [automatic extraction](./managing-memory#automatic-extraction) on a writable store to capture memories from the conversation. By default it runs every 5 turns, distilling facts client-side with a `ModelExtractor` that uses the agent's own model:
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:extraction_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:extraction"
```
</Tab>
</Tabs>

To change the cadence, swap the extractor, or extract server-side, see [Automatic Extraction](./managing-memory#automatic-extraction) on the Memory page.
...
```typescript
--8<-- "user-guide/sdk/memory/bedrock-knowledge-base_imports.ts:acl_store_imports"

--8<-- "user-guide/sdk/memory/bedrock-knowledge-base.ts:acl_store"
```
</Tab>
</Tabs>
