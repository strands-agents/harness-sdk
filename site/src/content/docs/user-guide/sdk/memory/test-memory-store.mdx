...
`TestMemoryStore` is a [`MemoryStore`](./managing-memory#stores) backed by a JSON file.
...
```typescript
--8<-- "user-guide/sdk/memory/test-memory-store_imports.ts:basic_imports"

--8<-- "user-guide/sdk/memory/test-memory-store.ts:basic"
```
</Tab>
</Tabs>

The store is writable by default, unlike a read-only managed store. So the [`add_memory`
tool](./managing-memory#memory-tools) and [automatic extraction](./managing-memory#automatic-extraction)
...
```typescript
--8<-- "user-guide/sdk/memory/test-memory-store_imports.ts:persistence_imports"

--8<-- "user-guide/sdk/memory/test-memory-store.ts:persistence"
```
</Tab>
</Tabs>
...
`TestMemoryStore` takes the [shared `MemoryStore` fields](./managing-memory#stores) (`name`,
...
```typescript
--8<-- "user-guide/sdk/memory/test-memory-store_imports.ts:search_and_add_imports"

--8<-- "user-guide/sdk/memory/test-memory-store.ts:search_and_add"
```
</Tab>
</Tabs>
...
Enable [automatic extraction](./managing-memory#automatic-extraction) to capture memories from
...
```typescript
--8<-- "user-guide/sdk/memory/test-memory-store_imports.ts:extraction_imports"

--8<-- "user-guide/sdk/memory/test-memory-store.ts:extraction"
```
</Tab>
</Tabs>
...
extractor, see [Automatic Extraction](./managing-memory#automatic-extraction) on the Memory page.
