...
To see what the SDK is doing internally, turn on its logging. Strands ships logging infrastructure that plugs into your application's existing setup.

<Tabs>
<Tab label="Python">

Strands Harness SDK uses Python's standard [`logging`](https://docs.python.org/3/library/logging.html) module. The SDK implements a straightforward logging approach:
...
Strands Harness SDK provides a simple logging infrastructure with a global logger that can be configured to use your preferred logging implementation.
...
```typescript
--8<-- "user-guide/sdk/observability-evaluation/logs.ts:basic_console"
```

**Using Pino:**

```typescript
--8<-- "user-guide/sdk/observability-evaluation/logs.ts:pino_setup"
```
...
The Strands Agents SDK logs information in several key areas. The following
example shows the logs you might see from an agent with a notebook tool:
...
With logging enabled, you see logs from different SDK components as the agent
processes the request, calls the notebook tool, and generates a response.
...
```
# Tool registration
DEBUG | strands.tools.registry | tool_name=<notebook> | registering tool
DEBUG | strands.tools.registry | tool_name=<notebook>, tool_type=<function>, is_dynamic=<False> | registering tool
DEBUG | strands.tools.registry | tool_name=<notebook> | loaded tool config
...
DEBUG | strands.tools.registry | tool_modules=<['notebook', 'weather']> | discovered
...
DEBUG | strands.tools.registry | tool_name=<notebook> | loaded dynamic tool config

# Tool execution
DEBUG | strands.event_loop.event_loop | tool_use=<calculator_tool_use_id> | streaming

# Tool hot reloading
DEBUG | strands.tools.registry | tool_name=<notebook> | searching directories for tool
DEBUG | strands.tools.registry | tool_name=<notebook> | reloading tool
DEBUG | strands.tools.registry | tool_name=<notebook> | successfully reloaded tool
```
...
```
DEBUG | strands.models.bedrock | config=<{'model_id': 'global.anthropic.claude-sonnet-5'}> | initializing
...
DEBUG model_id=<global.anthropic.claude-sonnet-5>, include_tool_result_status=<true> | auto-detected includeToolResultStatus
...
```typescript
--8<-- "user-guide/sdk/observability-evaluation/logs.ts:custom_logger"
```
</Tab>
</Tabs>
