...
Events for handling user barge-ins during model responses.

#### BidiBargeInEvent

Emitted when the user barges in during a response, as detected by voice activity detection.
...
:::note[BidiBargeInEvent vs Human-in-the-Loop Interrupts]
`BidiBargeInEvent` is different from [human-in-the-loop (HIL) interrupts](../interrupts.md). BidiBargeInEvent is emitted when the model detects user speech during audio conversations and automatically stops generating the current response. HIL interrupts pause agent execution to request human approval or input before continuing, typically used for tool execution approval. BidiBargeInEvent is automatic and audio-specific, while HIL interrupts are programmatic and require explicit handling.
:::
