...
- `stop_reason`: Why the response ended
  ...
  - `"interrupt"`: User interrupted the response
...
Emitted when the audio stream ends, including when interrupted. Buffered audio may still be playing. This event carries no audio and does not end the response or its transcript.
...
Emitted once when a user or assistant transcript ends, including on failure.
...
- `content_id`: Identifier for this transcript
- `error`: Optional failure details with `type` and `message`

The `event.error` property returns the original exception, or `None` on success. A failure marks the message incomplete without ending the conversation.
...
Signals an interruption to response generation or playback, typically when the user starts speaking. The bidirectional session continues.
...
```python
async for event in agent.receive():
    if event["type"] == "bidi_response_interrupt":
        print(f"Interrupted by {event['reason']}")
        # Audio output automatically cleared
        # Model ready for new input
```

:::note[Response and tool interrupts]
`BidiResponseInterruptEvent` applies to the current response or its playback. New input typically starts another response. Agent's [human-in-the-loop interrupts](../interrupts.md) pause an invocation to await input before resuming tool execution. BidiAgent does not yet support tool interrupts.
:::
...
async def main():
    ...
    async with BidiAgent(model=model) as agent:
        ...
        async for event in agent.receive():
            ...
            elif event["type"] == "bidi_response_interrupt":
                interruption_count += 1
                print(f"\nInterrupted (#{interruption_count})")
            
            elif event["type"] == "bidi_response_stop":
                if event["stop_reason"] == "interrupt":
