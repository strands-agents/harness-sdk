...
Response start and stop events bracket the assistant's audio, text, reasoning,
transcript, and tool requests. User transcription is independent and may arrive outside
these boundaries.

#### BidiResponseStartEvent

Emitted before a response's assistant audio, text, reasoning, transcript, and
tool-use events.
...
Each assistant audio stream has start, delta, and stop events sharing a `content_id`.
...
Text events carry written assistant responses. Reasoning events carry reasoning text
or thought summaries exposed by the model.
...
Transcript events describe user or assistant speech. Each event includes a `role`
and `content_id`.
...
Use deltas for live updates and block events for completed text, reasoning, and
...
Streamed messages in `agent.messages` include `BidiContentMetadata` under
