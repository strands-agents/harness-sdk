...
| Value | Meaning |
|-------|---------|
| `end_turn` | The model finished the response on its own |
| `barge_in` | The user cut in, or a new response began before this one ended |
| `tool_use` | The model handed off to a tool |
| `error` | The response or model stream ended with an error |
| `incomplete` | The session stopped while the response was still open |

`incomplete` is expected when a session ends mid-turn and is not itself an error.
