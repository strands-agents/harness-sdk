...
description: 'Trace, measure, and log BidiAgent sessions: connection lifecycle, time to first audio, interruptions, per-modality token usage, and reconnects.'
sidebar:
  label: "Observability"
languages: Python
experimental: true
...
A `BidiAgent` session is shaped differently from a request/response agent: one
connection spans multiple model responses, tool calls, and possibly reconnects, and
a response can end because the user interrupted it rather than because the model
finished. This page covers the traces, metrics, and logs specific to bidirectional
streaming. For the tracing, metrics, and logging concepts shared by every Strands
...
An interruption is recorded as a `bidi_response_interrupt` event on the session span with an `interruption.reason` attribute. The interrupted response separately closes with `gen_ai.response.finish_reason` set to `interrupt`.

To count interruptions, query the session span's `bidi_response_interrupt` events. To find which response ended, query response spans with an `interrupt` finish reason. Keeping the event on the session also captures barge-ins between response spans. For detection and playback behavior, see [Interruptions](interruption.md).
...
`BidiAgent` does not produce a metrics summary object. Interruption and reconnect counts
come through hooks, token usage comes through the event stream, and response latency is
...
class SessionStats(HookProvider):
    ...
    def on_interruption(self, event: BidiResponseInterruptEvent) -> None:
...
the check above counts successful recoveries. A rising interruption count usually points
to responses that run long for a voice interface; a rising reconnect count points to
sessions that frequently reach provider timeout conditions. The full list of lifecycle
events is in [Hooks](hooks.md).
...
4. **Watch the interruption rate.** A rising rate of barge-ins often points to responses
