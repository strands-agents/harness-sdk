...
text-only response has no such attribute. A model stream error closes the response
span with error status. Ending a session mid-turn also closes the response span
