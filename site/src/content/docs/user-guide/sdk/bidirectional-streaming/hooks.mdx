...
`BidiResponseStopEvent` carries `agent` and `response_id`. Hooks run in registration
order and finish before the corresponding streaming event reaches the consumer.
The hook mirrors model-reported completion: shutdown or a connection failure without
