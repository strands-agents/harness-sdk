...
:::tip[Quick setup]
The [ContextManager](../context-management) includes
built-in offloading strategies with stash and retrieval.
For most agents, pass
<Syntax
  py='context_manager="auto"'
  ts='contextManager: "auto"'
/> instead of configuring `ContextOffloader` separately.
:::

:::note
`ContextOffloader` is for agents that configure offloading
independently of
<Syntax
  py="context_manager"
  ts="contextManager"
...
<Syntax
  py="context_manager"
  ts="contextManager"
/> is set, its built-in strategies handle offloading and
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:getting_started"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:getting_started"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:custom_thresholds"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:custom_thresholds"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:in_memory_storage"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:in_memory_storage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:local_file_storage"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:local_file_storage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:s3_storage"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:s3_storage"
```
</Tab>
</Tabs>
...
With `InMemoryStorage`, there is no external access path, so the
built-in retrieval tool is the only way to access offloaded
content. Keep it enabled.
...
```typescript
--8<-- "user-guide/sdk/plugins/context-offloader_imports.ts:disable_retrieval_tool"

--8<-- "user-guide/sdk/plugins/context-offloader.ts:disable_retrieval_tool"
```
</Tab>
</Tabs>
