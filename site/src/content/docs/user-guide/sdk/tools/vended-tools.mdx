...
sourceLinks:
  ...
  - path: strands-py/src/strands/vended_tools/mcp_router/mcp_router.py
  ...
  - path: strands-py/src/strands/experimental/tools/_python_repl/_python_repl.py
---
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools.ts:basic_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:agent_with_vended_tools"
```
...
| [Python REPL](#python-repl-experimental) | Execute Python code in a sandboxed interpreter with persistent session state | Python |
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:file_editor_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:file_editor_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:http_request_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:http_request_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:notebook_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:notebook_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:notebook_persistence_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:notebook_state_persistence"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:notebook_custom_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:notebook_custom_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:bash_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:bash_example"
```
...
--8<-- "user-guide/sdk/tools/vended-tools.ts:bash_session"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:sleep_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:sleep_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:sleep_custom_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:sleep_custom_example"
```
...
Lets the model pause the agent loop and surface a message to the user for
human-in-the-loop input. Use this when the agent needs explicit confirmation,
additional information, or user approval before proceeding.

_Supported in: Node.js, modern browsers (TypeScript); all platforms (Python)._

:::note[Bidirectional streaming not supported]
...
:::note[Combining with HumanInTheLoop]
`HumanInTheLoop()` requires approval for every tool by default, so pairing it
with `handoff_to_user` prompts the user twice — once to approve the call, then
again to answer it. Allow-list the tool with
`HumanInTheLoop(allowed_tools=["handoff_to_user"])` / `new HumanInTheLoop({ allowedTools: ["handoff_to_user"] })`
so only the handoff itself prompts.
:::
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:handoff_to_user_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:handoff_to_user_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:stop_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:stop_example"
```
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:web_fetch_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:web_fetch_example"
```

Reading the full page as markdown:

```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:web_fetch_markdown_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:web_fetch_markdown_example"
```

Tighter response cap with a dedicated analyst model:

```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:web_fetch_custom_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:web_fetch_custom_example"
```
...
Lets your agent discover and communicate with remote [A2A (Agent-to-Agent) protocol](https://google.github.io/A2A/) agents.
...
The tool is stateless: a fresh `A2AAgent` is created on every call using the
`ClientConfig` configured for that endpoint.
Use ``make_a2a_client`` to control which endpoints the model may contact, with
per-endpoint authentication, and to tune size limits.
Requires `pip install 'strands-agents[a2a]'`.

_Supported in: Python._

:::caution[Endpoint security]
...
### Python REPL (Experimental)

> This tool is experimental and subject to change in future
> revisions without notice.

Runs model-generated Python code in a [Monty](https://github.com/pydantic/monty) sandbox. Session state (variables, imports, and definitions) persists across calls via agent state, so the model can build on earlier results without redefining everything each turn. Pass `reset_state=True` to start from a clean namespace.

_Supported in: Python only._

:::note[Install required]
`python_repl` requires the optional `python-repl` extra:
...
:::note[Security posture]
Monty is a language-level sandbox, not an OS-level one. The bytecode has no
host operations, so untrusted code cannot reach the filesystem, network, or
environment. For an OS boundary around the tools the code calls, run the agent
in a [sandboxed environment](../sandbox/index.mdx).
:::
...
agent("Use python_repl to compute the first 10 Fibonacci numbers and print them.")
```
...
```python
from strands import Agent
from strands.experimental.tools import make_python_repl
...
```typescript
--8<-- "user-guide/sdk/tools/vended-tools-imports.ts:combined_import"

--8<-- "user-guide/sdk/tools/vended-tools.ts:combined_tools_example"
```
