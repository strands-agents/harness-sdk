...
A Swarm hands a pool of specialized agents to a single orchestrator and lets them hand off work to one another autonomously, with no fixed path between them. Reach for it when a task benefits from several perspectives and you want the agents, rather than a developer-defined graph, to decide who works next.

Unlike a sequential or hierarchical arrangement, a Swarm coordinates agents through shared context and working memory:

- Agents hand off to one another autonomously, without central control.
- Every agent reads the full task context and the knowledge earlier agents contributed.
- Tasks distribute dynamically based on each agent's expertise.
- Text, images, and other content types are all supported as input.

## How Swarms Work

A group of specialized agents handing work between themselves can cover a problem more thoroughly than one agent alone. Each agent in a Swarm:
...
```typescript
--8<-- "user-guide/sdk/multi-agent/swarm.ts:swarm_team"
```
</Tab>
</Tabs>
...
Detection only runs once the window is full, so `maxSteps` must be greater than
`repetitiveHandoffDetectionWindow` for it to fire. When `maxSteps` is less than or equal
to the window, the `maxSteps` limit is reached first and takes precedence.
...
```typescript
--8<-- "user-guide/sdk/multi-agent/swarm.ts:multimodal"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/swarm.ts:streaming"
```
</Tab>
</Tabs>

See the [stream event types](../streaming/events.md#multi-agent-events) reference for details on all multi-agent event types.
...
```typescript
--8<-- "user-guide/sdk/multi-agent/swarm.ts:results"
```
</Tab>
</Tabs>
...
7. **Use multi-modal inputs**: Pass ContentBlocks for rich inputs including images
...
**Repetitive handoff scope**: Python's detection window is restored across an
interrupt-and-resume of the same task, so a ping-pong that spans a resume is still
caught. TypeScript's window is scoped to the current invocation and resets on every
resume, so a ping-pong that alternates with interrupts across several resumes is not
detected. Set `maxSteps` if your swarm can interrupt mid-task. This is the opposite of
`maxSteps` itself, whose step count TypeScript does carry forward across resumes.
