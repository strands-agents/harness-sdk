...
Strands provides three ways to implement agents as tools: pass agents directly in the `tools` array for the simplest setup, use <Syntax py=".as_tool()" ts=".asTool()" /> to customize tool name, description, or context behavior, and use the `@tool` decorator or `tool()` function for full control over how the agent is invoked.
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:direct_passing"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:as_tool_customization"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:as_tool_context"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:research_assistant"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:multiple_specialists"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:orchestrator"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:usage"
```
</Tab>
</Tabs>
...
```typescript
--8<-- "user-guide/sdk/multi-agent/agents-as-tools.ts:delegation_basic"
```
</Tab>
</Tabs>
...
The [Teacher's Assistant](/examples/) example demonstrates an orchestrator agent that routes student queries to specialized agents for math, English, language translation, computer science, and general knowledge.
