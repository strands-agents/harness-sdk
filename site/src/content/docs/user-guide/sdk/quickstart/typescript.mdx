...
description: "Get an AI agent running in TypeScript in under 5 minutes. Install the SDK, pick a model provider, run the agent, then give it a tool."
sidebar:
  label: "TypeScript"
tags: [quickstart]
redirectFrom:
...
<CopyPromptButton language="typescript" />

## Install the Strands Harness SDK

Make sure you have Node.js 22+ and npm installed. See the
[npm docs](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm) if you need
...
--8<-- "user-guide/sdk/quickstart/typescript.ts:run-bedrock"
```
...
--8<-- "user-guide/sdk/quickstart/typescript.ts:run-anthropic"
```
...
--8<-- "user-guide/sdk/quickstart/typescript.ts:run-openai"
```
...
--8<-- "user-guide/sdk/quickstart/typescript.ts:run-google"
```
...
[See all supported model providers](../model-providers/).
...
--8<-- "user-guide/sdk/quickstart/typescript.ts:custom-tool"
```
...
```typescript
--8<-- "user-guide/sdk/quickstart/typescript.ts:tool-bedrock"
```
...
```typescript
--8<-- "user-guide/sdk/quickstart/typescript.ts:tool-anthropic"
```
...
```typescript
--8<-- "user-guide/sdk/quickstart/typescript.ts:tool-openai"
```
...
```typescript
--8<-- "user-guide/sdk/quickstart/typescript.ts:tool-google"
```
...
:::note
The `tool()` function also accepts plain JSON Schema objects instead of Zod. See [Creating Custom Tools](../tools/custom-tools) for details.
:::
...
traces. The [Agent Loop](../agents/agent-loop.md) explains the cycle above, and
[Observability](../observability-evaluation/observability.md) covers reading traces and
...
- [State](../agents/state.md) - how agents keep context across a conversation
- [Streaming](../streaming/) - stream events to a UI with async iterators
