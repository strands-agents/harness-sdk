...
import type { MockMessageContentBlock, MockMessageTurn, MockMessageTurnOptions } from './types.js'
 ...
 */
type Turn =
  | { type: 'content'; content: MockMessageContentBlock[]; stopReason: StopReason; usage?: Usage }
 ...
 * A single configured turn repeats; multiple turns are consumed in order.
 * Skips media, JSON, guard-content, and tool-result blocks rather than simulating them.
 *
 * @example
 * ```typescript
...
export class MockMessageModel extends Model<BaseModelConfig> {
  ...
  /**
   * Number of streams that began consumption, including errors and exhausted attempts.
   * Creating an iterator without consuming it does not count as a call.
   */
   ...
   * ```
   */
  addTurn(turn: MockMessageTurn, options?: MockMessageTurnOptions): this {
   ...
   * @throws Error - If no turns remain or the configured turn is an error.
   */
  ...
  private async *_generateEventsForContent(
    content: MockMessageContentBlock[],
   ...
   */
  private _createTurn(turn: MockMessageTurn, explicitStopReason?: StopReason, usage?: Usage): Turn {
   ...
   */
  private _deriveStopReason(content: MockMessageContentBlock[]): StopReason {
   ...
   */
  private async *_generateEventsForBlock(block: MockMessageContentBlock): AsyncGenerator<ModelStreamEvent> {
    switch (block.type) {
      case 'textBlock':
      ...
      default: {
        ...
        throw new Error(`Unknown content block type: ${(_exhaustive as MockMessageContentBlock).type}`)
      }
    }
  }
}
