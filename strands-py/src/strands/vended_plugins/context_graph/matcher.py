...
The embedding round trip and the cache that makes it affordable are reused verbatim from
:mod:`strands.vended_plugins._embedding`. New here are the protocol and the adapter.

Asymmetric by construction: the short side is a question and the long side is a description of up to
``description_tokens``, which asks for ``"query"`` against ``"document"``. Passing ``"clustering"`` where ``"query"``
was right does not fail, it silently returns a worse vector. Hence the purpose is a parameter and the cache is keyed by
the ``(purpose, text)`` pair rather than by text alone.

Unlike the ``Reranker`` protocol, which raises by contract, :class:`SimilarityMatcher` reports unavailability by
...
class SimilarityMatcher(Protocol):
    ...
    def score(self, question: str, descriptions: Sequence[str]) -> Sequence[float]:
        ...
        Args:
            ...
            descriptions: The Cards' descriptions. Embedded under purpose ``"document"``. Must not be mutated, reordered
                or resized.
...
class EmbeddingSimilarityMatcher:
    ...
    One embedding round per turn, the question under ``"query"`` and the descriptions under ``"document"``, and within
    it only the uncached texts reach Bedrock. The cache is keyed by the ``(purpose, text)`` pair, so an unchanged
    description costs nothing next turn.

    Never raises: :class:`~strands.vended_plugins._embedding.EmbeddingError` becomes an empty sequence plus one debug
    log carrying ``exc_info``. A caller whose correctness depends on the score should use the embedder directly and let
    the error through.

    Args:
        model_id: Bedrock embedding model. Defaults to a multilingual model.
        boto_session: Optional boto3 session. The client is built lazily on first use, so construction performs no I/O.
        boto_client_config: Optional botocore client configuration.
        region_name: Region, used only when no session is supplied.
        embedder: Pre-built embedder, taking precedence over the arguments above. Lets an application share one client
            and one cache across matchers.
    """
    ...
    def score(self, question: str, descriptions: Sequence[str]) -> Sequence[float]:
        ...
        Args:
            ...
            descriptions: The Cards' descriptions, embedded under ``"document"``. Read only: elements, order and size
                are all left untouched.

        Returns:
            Exactly ``len(descriptions)`` values in ``[0.0, 1.0]``, aligned by index. Empty when embedding is
    ...
    def vectors(self, descriptions: Sequence[str]) -> Sequence[Sequence[float]]:
        ...
        Free after :meth:`score`: the embedder caches by ``(purpose, text)``, so these are the vectors already computed
        for the note and no call goes out. The ``similar`` edge between two Cards is measured from that cache and never
        from a remote call, so a caller that never fills it can create no such edge.

        Args:
            descriptions: The Cards' descriptions. Read only.

        Returns:
            One vector per description, in order. Empty when embedding is unavailable, which leaves the cache as it was
            and the edge unmeasurable rather than asserted absent.
        """
        if not descriptions:
            return []
        try:
    ...
    def _embed_both(self, question: str, texts: list[str]) -> tuple[list[float], list[list[float]]]:
        ...
        Two calls and not one because the asymmetry forces it: the question goes under ``"query"``, the descriptions
        under ``"document"``, and one Cohere call carries one ``input_type``.
        ...
        Raises:
            EmbeddingError: Propagated from either call, for :meth:`score` to turn into an empty sequence.
        """
        # Materialized here so neither worker has to. ``boto3.Session.client`` is not safe to call from two threads at
        # once and the real embedder builds its client on first use, so the first scored turn is where that race would
        # live. Read through ``getattr`` because the embedder is duck-typed: an in-memory double has no client to build.
        ...
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="context-graph-embed") as pool:
            ...
            # The question first: the smaller call, so its failure is reported without waiting on the larger one.
