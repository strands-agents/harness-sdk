...
@pytest.mark.asyncio
async def test_resume_after_non_suspending_entry_point_runs_join_once(max_concurrency):
    """The whole dispatch batch is in flight before any entry point can checkpoint (#3633)."""

    def build_graph():
        builder = GraphBuilder()
        for name in ("left", "right", "join"):
            builder.add_node(create_mock_agent(name, f"{name} done"), name)
        for name in ("left", "right"):
            builder.set_entry_point(name)
            builder.add_edge(name, "join")
        return builder.set_max_concurrency(max_concurrency).build()

    graph = build_graph()
    captured = {}

    def capture_first_entry_point(event):
        if event.node_id in ("left", "right") and not captured:
            sibling = "right" if event.node_id == "left" else "left"
            captured["sibling"] = sibling
            captured["status"] = event.source.nodes[sibling].execution_status
            captured["payload"] = event.source.serialize_state()

    graph.add_hook(capture_first_entry_point, AfterNodeCallEvent)
    await graph.invoke_async("two entry points")

    tru_checkpoint = (captured["status"], captured["payload"]["next_nodes_to_execute"])
    exp_checkpoint = (Status.EXECUTING, [captured["sibling"]])
    assert tru_checkpoint == exp_checkpoint

    restored = build_graph()
    restored.deserialize_state(captured["payload"])
    result = await restored.invoke_async("two entry points")
    tru_execution = (result.status, restored.nodes["join"].executor.stream_async.call_count)
    exp_execution = (Status.COMPLETED, 1)
    assert tru_execution == exp_execution
    join_input = str(restored.nodes["join"].executor.stream_async.call_args.args[0])
    assert "From left:" in join_input
    assert "From right:" in join_input
