...
@pytest.mark.asyncio
async def test_audio_io_input(audio_input):
...
def test_audio_io_input_configs(pyaudio_module, py_audio, audio_input):
...
async def test_audio_io_output_interrupt(audio_output):
    ...
    await audio_output(audio_event)
    interrupt_event = BidiResponseInterruptEvent(reason="user_speech")
...
async def test_audio_io_start_rejects_model_without_audio_capability(pyaudio_module, direction):
    agent = unittest.mock.MagicMock()
    agent.model = object()
    audio_io = AudioIO()
    io = audio_io.input() if direction == "input" else audio_io.output()

    with pytest.raises(TypeError, match="AudioIO requires a model that implements AudioCapable"):
...
async def test_audio_io_start_rejects_unsupported_encoding(pyaudio_module, agent, direction, format):
...
def test_audio_processor_uses_defaults(audio_processor):
    audio_io = AudioIO(audio_processor=audio_processor)
...
def test_config_rejects_negative_delay():
    with pytest.raises(ValueError, match="stream_delay_ms"):
        AudioIO(audio_processor=AudioProcessorConfig(stream_delay_ms=-5))


def test_config_rejects_excessive_delay():
    with pytest.raises(ValueError, match="stream_delay_ms"):
        AudioIO(audio_processor=AudioProcessorConfig(stream_delay_ms=5000))
...
def test_config_allows_disabling_echo_cancellation():
    ...
    audio_io = AudioIO(audio_processor=config)
...
def test_config_rejects_stream_delay_when_echo_cancellation_is_off():
    with pytest.raises(ValueError, match="requires echo cancellation"):
        AudioIO(audio_processor=AudioProcessorConfig(echo_cancellation=False, stream_delay_ms=10))


# ---------------------------------------------------------------------------
# AudioIO construction and enablement
# ---------------------------------------------------------------------------
...
def test_audio_processor_shared_between_input_and_output():
    ...
    audio_io = AudioIO(audio_processor=audio_processor_config)
    input_ = audio_io.input()
    output = audio_io.output()
    ...
    try:
        with unittest.mock.patch("builtins.__import__", side_effect=import_without_pywebrtc):
            with pytest.raises(
                ...
                AudioIO(audio_processor=AudioProcessorConfig())
    finally:
        sys.modules[module_name] = module


def test_audio_config_is_keyword_only():
    # AudioIO takes only keyword configuration.
    with pytest.raises(TypeError):
        AudioIO(AudioProcessorConfig())  # type: ignore[misc]
...
async def test_input_keeps_configured_buffer_when_processing_off(py_audio, aec_agent):
    audio_io = AudioIO(input_frames_per_buffer=1024)
    input_ = audio_io.input()
    await input_.start(aec_agent)
...
def test_mic_buffer_bounded_to_reference_horizon_when_ec_on(mock_audio_processor):
    ...
    audio_io = AudioIO(audio_processor=AudioProcessorConfig(), input_buffer_size=2)
    input_ = audio_io.input()
...
def test_mic_buffer_defaults_to_processing_limit():
    audio_io = AudioIO(audio_processor=AudioProcessorConfig(), input_buffer_size=None)
    input_ = audio_io.input()
...
def test_mic_buffer_rejects_invalid_size_when_ec_on(input_buffer_size):
    with pytest.raises(ValueError, match="input_buffer_size"):
        AudioIO(audio_processor=AudioProcessorConfig(), input_buffer_size=input_buffer_size)
...
def test_mic_buffer_uses_configured_size_when_ec_off(pyaudio_module, mock_audio_processor):
    ...
    audio_io = AudioIO(audio_processor=AudioProcessorConfig(echo_cancellation=False), input_buffer_size=7)
    input_ = audio_io.input()
...
def test_frames_per_buffer_rejected_when_ec_on(config):
    with pytest.raises(ValueError, match="calculated automatically"):
        AudioIO(audio_processor=AudioProcessorConfig(), **config)
...
async def test_output_keeps_configured_buffer_when_processing_off(py_audio, aec_agent):
    audio_io = AudioIO(output_frames_per_buffer=2048)
    output = audio_io.output()
    await output.start(aec_agent)
...
async def test_output_clears_reference_on_interruption(py_audio, aec_agent, mock_audio_processor):
    from strands.experimental.bidi.types import BidiAudioDeltaEvent, BidiResponseInterruptEvent
    ...
    await output(BidiResponseInterruptEvent(reason="user_speech"))
