...
Out of the box, Strands provides `AudioIO` to connect your microphone and speakers to the bidi-agent. It captures microphone audio, streams it to the model in real time, and plays responses through the speakers using [PyAudio](https://pypi.org/project/PyAudio/).

Installation Required

`AudioIO` requires the `bidi-io` and `bidi-pyaudio` extras plus the PortAudio system library. Install PortAudio first, then install the extras:
